...
// newIndex creates an index for the given file. We create the index and
// save the current size of the file so we can track the amount of data in the
// index file as we add entries. We grow the file to the max index size before
...
// always the offset of the index's first entry, 1 is the second entry, and so
// on. We use relative offsets to reduce the size of indexes by storing offsets
// as uint32s. If we used absolute offsets, we'd have to store the offsets as
// uint64s and require four more bytes for each entry. cuz scaling
...
// Write appends the given offset and position to the index. First, we
// validate that we have space to write the entry. If there's space, we then
...
// Then it truncates the persisted file to the amount of data that's actually

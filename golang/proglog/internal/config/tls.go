...
// SetupTLSConfig helps with the following three configurations:
//  * Client *tls.Config is set up to verify the server's certificate with the
//    client's by setting the *tls.Config's RootCAs.
//  * Client *tls.Config is set up to verify the server's certificate and all
//    the server to verify the client's certificate by setting its RootCAs and its
//    Certificates
